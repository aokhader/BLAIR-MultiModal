...
We extend the BLAIR (Bridging Language and Items for Retrieval) framework by incorporating **image embeddings** through the **CLIP vision encoder**, creating a multimodal item representation we call **BLaIR-CLIP**. Additionally, this model handles cold-start naturally because it relies on content rather than collaborative signal, which is an improvement over the standard baselines.
